...
unsigned int glyphVAO, glyphVBO;
...
int main()
{
	...
	// для текста glyphVAO и glyphVBO
	{
	...
	Shader glyphShader("../midnight/glyph.vs", "../midnight/glyph.fs", NULL); // Шейдер для текста


	unsigned int floor = loadTexture("C:\\Users\\Tyuri\\Documents\\GitHub\\res\\floor.png");
	...
	// Загрузка шрифта
	{
		...
		std::string font_name = "../res/fonts/a_AlternaSw.ttf";
		if (font_name.empty())
		{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Отрисовка на экран
		{
			...
			RenderText(glyphShader, "Урааа, Получилось!!!", 50.0f, 50.0f, 1.0f, glm::vec3(0.5, 0.8f, 0.2f));
			RenderText(glyphShader, "Удивительно..!", static_cast<float>(SCR_WIDTH) - 300.0f, static_cast<float>(SCR_HEIGHT) - 100.0f, 0.5f, glm::vec3(0.3, 0.7f, 0.9f));
			glDisable(GL_BLEND);
